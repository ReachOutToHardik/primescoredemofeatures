  ...
  return (
    <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-12 pb-24 text-brandNavy bg-night">
      ...
      <section className="mt-24">
        ...
        {/* Mobile Comparison Cards (Screenshot 1 alignment) */}
        <div className="mt-10 grid gap-4 sm:hidden">
          {comparisonRows.map((row) => (
            <div key={row.k} className="rounded-2xl border border-brandNavy/8 bg-white p-5 shadow-sm">
              <div className="text-sm font-bold text-brandNavy border-b border-brandNavy/5 pb-2 mb-3 leading-snug">{row.k}</div>
              <div className="space-y-2.5 text-xs">
                <div className="flex items-center justify-between">
                  <span className="text-textSecondary">Primescore</span>
                  <span className="font-bold text-brandGreen flex items-center gap-1">
                    <span>✓</span> {row.a}
                  </span>
                </div>
                <div className="flex items-center justify-between border-t border-dashed border-brandNavy/5 pt-2">
                  <span className="text-textSecondary">DIY</span>
                  <span className="text-textSecondary font-medium">{row.b}</span>
                </div>
                <div className="flex items-center justify-between border-t border-dashed border-brandNavy/5 pt-2">
                  <span className="text-textSecondary">Other platforms</span>
                  <span className="text-textSecondary font-medium">{row.c}</span>
                </div>
              </div>
            </div>
          ))}
